...
def analyze_errors():
    ...
    # Prepare Data
    # The target in the dataframe is 'transformed_engagement' (Sqrt)
    # We also want the original 'engagement' if possible, or we reconstruct it
    
    y_trans = df['transformed_engagement'].values
    
    # Drop target and Id for X
    cols_to_drop = ['engagement', 'transformed_engagement', 'log_engagement', 'Id']
    X = df.drop(columns=cols_to_drop, errors='ignore')
    
    # Replicate the split from training
    ...
    # Get original indices to trace back to authors/features if needed
    ...
    # --- Analysis 2: Feature Correlations with Error ---
    ...
    # --- Analysis 3: Feature 2 Analysis ---
    if 'feature2' in analysis_df.columns:
        print("\n--- Error by Feature 2 (Sentiment?) ---")
        # feature2 might be scaled/encoded. Let's check if it's raw or processed.
        # In 02_process_data, we kept it numerical. But it might be scaled by RobustScaler in the pipeline.
        # However, X_val here is the raw input to the pipeline (from parquet), so it should be unscaled!
        # Wait, 02_process_data saves the processed features. 
        # 03_train_model applies the scaler.
        # So X_val here has the values as they came out of 02_process_data.
        
        # Check unique values of feature2 in validation
